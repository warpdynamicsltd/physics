...
As a direct conclusion from the above and (\ref{invariance-of-trace}) it follows that trace is an intrinsic property of a linear operator independent on choice of basis.
...
\begin{lemma}
Let $\Field$ be a field. If $A\in M_p(\Field)$ and $B\in M_q(\Field)$ where $p,q\geq 1$ and $C\in M_{p + q}(\Field)$ such that
\begin{equation}
C = A \oplus 0_q + 0_p \oplus B = \begin{bmatrix}
    A &  0 \\
    0 & B \\
\end{bmatrix},
\end{equation}
then
\begin{equation}
\label{direct-sum-determinants}
\det(A - \lambda I)\det(B - \lambda I) = \det(C - \lambda I).
\end{equation}
\end{lemma}
\begin{proof}
Note that for $\lambda = 0$ (\ref{direct-sum-determinants}) holds. Let's assume that $\lambda\not=0$. Let 
$A' =  A \oplus 0_q $ and $B' = 0_p \oplus B$.
\begin{align*}
& (-\lambda)^q \det(A - \lambda I) (-\lambda)^p \det(B - \lambda I)  = \det(A' - \lambda I)\det(B' - \lambda I) = \\
& \det((A' - \lambda I )(B' - \lambda I)) = \det(A'B' -\lambda A' - \lambda B' + \lambda^2 I) = \\
& \det((-\lambda)(C - \lambda I)) = (-\lambda)^{p + q} \det(C - \lambda I).
\end{align*}
\end{proof}

The above can be alternatively proved using Theorem \ref{det-0}.

\begin{definition}
Let $V$ be a vector space and $A:V\to V$ be a linear operator.
\begin{equation}
\mathcal{E}_A(\lambda) = \{x\in V: Ax = \lambda x\}.
\end{equation}
\end{definition}

\begin{theorem}
Let $V$ be a vector space over field $\C$ with $\dim V < +\infty$. If $A:V\to V$ is a linear operator, then there exist numbers $\lambda_1, \dots, \lambda_m\in\C$ such that $\mathcal{E}_A(\lambda_k)\not = \emptyset$ for $k=1,\dots, m$ and
\begin{equation}
V = \bigoplus_{k=1}^m \mathcal{E}_A(\lambda_k).
\end{equation}
Moreover,
\begin{equation}
\det A = \prod_{k=1}^m \lambda_k^{\dim \mathcal{E}(\lambda_k)},
\end{equation}
and
\begin{equation}
\tr A = \sum_{k=1}^m {\dim \mathcal{E}(\lambda_k)}\cdot \lambda_k.
\end{equation}
\end{theorem}
...
\begin{definition}
$(\mathcal{N}, <)$ a strictly partially ordered set will be called a finite net iff $\mathcal{N}$ is finite.
\end{definition}

\begin{definition}
Let $\mathcal{N}$ be a finite net. Let $\alpha\in \mathcal{N}$
\begin{equation}
\text{par}(\alpha) \stackrel{def}{=} \{ \beta\in \mathcal{N}: \beta < \alpha \text{ and } \forall(\gamma\in\mathcal{N}) \neg(\beta<\gamma<\alpha)\}.
\end{equation}

\begin{equation}
\text{child}(\alpha) \stackrel{def}{=} \{ \beta\in \mathcal{N}: \beta > \alpha \text{ and } \forall(\gamma\in\mathcal{N}) \neg(\beta > \gamma >\alpha)\}.
\end{equation}

\begin{equation}
\text{input}(\mathcal{N}) \stackrel{def}{=} \{\alpha\in\mathcal{N}: \text{par}(\alpha)=\emptyset\}
\end{equation}

\begin{equation}
\text{output}(\mathcal{N}) \stackrel{def}{=} \{\alpha\in\mathcal{N}: \text{child}(\alpha)=\emptyset\}
\end{equation}
\end{definition}
Let $\mathcal{N}$ be a finite net. $\mathcal{N}$ is a connected in-out net iff 
\begin{equation}
\text{input}(\mathcal{N}) < \text{output}(\mathcal{N}).
\end{equation}

\begin{equation}
\begin{cases}\
& s_\alpha = y_\alpha = x_\alpha \text{ for } \alpha\in\text{input}(\mathcal{N}),\\
& s_\alpha = \sum_{\beta\in\text{par}(\alpha)} w_{\beta\alpha} y_\beta \text{ for } \alpha\not\in\text{input}(\mathcal{N}),\\
& y_\alpha = f_\alpha(s_\alpha) \text{ for } \alpha\not\in\text{input}(\mathcal{N}).
\end{cases}
\end{equation}

For chosen $\beta_0\in\text{par}(\alpha_0)$ let $w$ denote $w_{\beta_0\alpha_0}$. 




\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = f'(s_\alpha) \cfrac{\partial y_\alpha}{\partial w}.
\end{equation}

Then note that for $\alpha \not= \alpha_0$

\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = f'(s_\alpha) \sum_{\beta\in\text{par}(\alpha)} w_{\beta\alpha} \cfrac{\partial y_\beta}{\partial w}.
\end{equation}

and

\begin{equation}
\cfrac{\partial y_{\alpha_0}}{\partial w} = f'(s_{\alpha_0}) y_{\beta_0}.
\end{equation}

Note that 
\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = 0 \text{ for } \alpha \not=\alpha_0 \text{ and } \neg(\alpha_0 < \alpha). 
\end{equation}

Let 
\begin{equation}
B_{\beta\alpha} \stackrel{def}{=} f'(s_\alpha) w_{\beta\alpha}.
\end{equation}

Note than for $\alpha > \alpha_0$, we have

\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = \sum_{\beta\in\text{par}(\alpha)} B_{\beta\alpha} \cfrac{\partial y_\beta}{\partial w}.
\end{equation}

Note that 
\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = \sum_{\beta\in\text{par}(\alpha) \text{ and } \beta \geq \alpha_0} B_{\beta\alpha} \cfrac{\partial y_\beta}{\partial w}.
\end{equation}

Thus

\begin{multline}
\cfrac{\partial y_\alpha}{\partial w} = 
\sum B_{\alpha_k\alpha} B_{\alpha_{k-1}\alpha_k} 
 \dots B_{\alpha_1\alpha_2} B_{\alpha_0 \alpha_1}
 \cfrac{\partial y_{\alpha_0}}{\partial w} \\
 = \cfrac{\partial y_{\alpha_0}}{\partial w}\sum B_{\alpha_k\alpha} B_{\alpha_{k-1}\alpha_k} 
 \dots B_{\alpha_1\alpha_2} B_{\alpha_0 \alpha_1},\\
\end{multline}
where summation is over all paths $\alpha_0, \alpha_1, \dots, \alpha_k, \alpha$ which connects $\alpha_0$ and $\alpha$.

Let's define

\begin{equation}
\begin{cases}
A(\alpha;\zeta) = \sum_{\gamma\in\text{child}(\zeta)} B_{\zeta\gamma} A(\alpha; \gamma) \text{ for } \zeta < \alpha, \\
A(\alpha;\alpha) = 1.
\end{cases}
\end{equation}
...
Note that 
\begin{equation}
A(\alpha; \alpha_0) = \sum B_{\alpha_k\alpha} B_{\alpha_{k-1}\alpha_k} 
 \dots B_{\alpha_1\alpha_2} B_{\alpha_0 \alpha_1},\\
\end{equation}
where summation is over all paths $\alpha_0, \alpha_1, \dots, \alpha_k, \alpha$ which connects $\alpha_0$ and $\alpha$.
Thus
...
\begin{equation}
\cfrac{\partial y_\alpha}{\partial w} = f'(s_{\alpha_0}) y_{\beta_0} A(\alpha; \alpha_0).
\end{equation}

\subsection{Attention Head}

Lets consider input as 3 sequences of row vectors
\begin{multline}\\
\mathbf{x}^q_i\in \R^{d^q_{\text{emb}}}$ for $i=1, \dots, n, \\
\mathbf{x}^k_i\in \R^{d^k_{\text{emb}}}$ for $i=1, \dots, n, \\
\mathbf{x}^v_i\in \R^{d^v_{\text{emb}}}$ for $i=1, \dots, n, \\
\end{multline}
Let
\begin{equation}
X^q \stackrel{def}{=} 
...
\begin{multline}
\\
\dim X^q = n \times d^q_{\text{emb}},\\
\dim X^k = n \times d^k_{\text{emb}},\\
\dim X^v = n \times d^v_{\text{emb}}.\\
\end{multline}.

Consider 3 matrices of weights:
\begin{multline}
\\
W^Q \text{ with } \dim W^Q = d^q_{\text{emb}} \times d_q,\\
W^K \text{ with } \dim W^K = d^k_{\text{emb}} \times d_k,\\
W^V \text{ with } \dim W^V = d^v_{\text{emb}} \times d_v.\\
\end{multline}

Let's define
\begin{multline}
\\
Q \stackrel{def}{=} X^q W^Q (\text{note that }\dim Q = n \times d_q),\\
K \stackrel{def}{=} X^k W^K (\text{note that }\dim K = n \times d_k),\\
V \stackrel{def}{=} X^v W^V (\text{note that }\dim V = n \times d_v).\\
\end{multline}
...
For the next step, we will require $d_k = d_q$.

Note that
\begin{equation}
QK^T = \begin{bmatrix}
    (\mathbf{x}^q_1 W^Q)\cdot (\mathbf{x}^k_1 W^K) &  \dots  & (\mathbf{x}^q_1 W^Q)\cdot (\mathbf{x}^k_n W^K) \\
    \vdots &  \ddots & \vdots \\
    (\mathbf{x}^q_n W^Q)\cdot (\mathbf{x}^k_1 W^K) &  \dots  & (\mathbf{x}^q_n W^Q)\cdot (\mathbf{x}^k_n W^K) 
\end{bmatrix}
\end{equation}

Let's note that $(\mathbf{x}^q_i W^Q)\cdot (\mathbf{x}^k_j W^K)$ is maximised, when vectors $\mathbf{x}^q_i W^Q$ and $\mathbf{x}^k_j W^K$ are on the same line, which is certain measure of similarity.

Thus our heuristic interpretation of $n\times n$ matrix

\begin{equation}
\text{softmax}(\cfrac{1}{\sqrt{d_k}} QK^T) \approx  \begin{bmatrix}
    0 &  \dots & 0 & 0  & 1 & 0 & \dots & 0 \\
    \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
    0 &  \dots & 0 & 1 & 0 & 0 & \dots & 0 \end{bmatrix},
\end{equation}

where for each $i$-th row $1$ is in $j$-th column when a query $\mathbf{x}^q_i W^Q$ was most similar to a key $\mathbf{x}^k_j W^K$.

Thus the matrix of dimension $n \times d_v$
\begin{equation}
\text{softmax}(\cfrac{1}{\sqrt{d_k}} QK^T)V \approx 
\begin{bmatrix}
\mathbf{x}^v_{j_1} W^V\\
\vdots \\
\mathbf{x}^v_{j_n} W^V \\
...
where for each $m=1, \dots, n$ for query $\mathbf{x}^q_m W^Q$ the most similar key in dot product similarity is $\mathbf{x}^k_{j_m} W^K$.

\begin{definition}[Attention]
\begin{equation}
\text{Attention}(X^q, X^k, X^V) \stackrel{def}{=} \text{softmax}(\cfrac{1}{\sqrt{d_k}} QK^T)V.
\end{equation}
\end{definition}

\begin{definition}[Masked attention]
\begin{equation}
\text{Attention}(X^q, X^k, X^V) \stackrel{def}{=} \text{softmax}(\cfrac{1}{\sqrt{d_k}} QK^T + M)V.
\end{equation}
\end{definition}
...
\end{bmatrix}
\end{equation}
this will replace with zeros all vectors for $j_m \geq m$ for $m=1, \dots, n$ 
in result
\begin{equation}
\begin{bmatrix}
\mathbf{x}^v_{j_1} W^V\\
...
\end{bmatrix}.
\end{equation}
If you are concerned by infinities in $M_{\text{causal}}$ just think about some extremely big real values.
\end{document}