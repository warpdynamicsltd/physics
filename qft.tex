...
Although we work in units where $\hbar = 1$, we require some normalisation constant $\mathcal{C}_{\pi,1}$ depending on $L$ and $N$, wchose nature will become clear later in this subsection.
...
\exp\bigg[ -i \mathcal{C}_{\pi, 1} \sum_{\bs{x}\in\mathcal{U}} \phi(\bs{x})\pi(\bs{x})\bigg].
\end{equation}

Our goal now will be to find constants $\mathcal{C}_{\pi, 1}$ and $\mathcal{C}_{\pi, 2}$ for which $\bra{\pi_1}\ket{\pi_2} = \delta(\pi_1 - \pi_2)$. We want to have both bases normalised in the same way.
...
\exp\bigg[ -i \mathcal{C}_{\pi, 1}\sum_{\bs{x}\in\mathcal{U}} \phi(\bs{x})\big(\pi_2(\bs{x})
- \pi_1(\bs{x})\big)\bigg] = \\
& \mathcal{C}^2_{\pi,2} \prod_{\bs{x}\in \mathcal{U}} \int d\phi(\bs{x}) 
\exp\bigg[ -i \mathcal{C}_{\pi, 1} \phi(\bs{x})\big(\pi_2(\bs{x})
- \pi_1(\bs{x})\big)\bigg] = \\
& \mathcal{C}^2_{\pi,2} \prod_{\bs{x}\in \mathcal{U}} 2\pi
\delta\bigg(\mathcal{C}_{\pi, 1}\big(\pi_2(\bs{x}) 
- \pi_1(\bs{x})\big)\bigg) = 
\mathcal{C}^2_{\pi,2} \prod_{\bs{x}\in \mathcal{U}} \cfrac{2\pi}{\mathcal{C}_{\pi, 1}}
\delta\big(\pi_2(\bs{x}) - \pi_1(\bs{x})\big) = \\
& \mathcal{C}^2_{\pi,2}\bigg(\cfrac{2\pi}{\mathcal{C}_{\pi, 1}} \bigg)^N \delta(\phi_2 - \phi_1).
\end{align*}

We need to get
\begin{equation}
\lim_{L\to \infty}\lim_{N\to \infty} \mathcal{C}^2_{\pi,2} \bigg(\cfrac{2\pi}{\mathcal{C}_{\pi, 1}} \bigg)^N = 1.
\end{equation}
...
Remeberining that $\Delta x = \cfrac{2L}{N}$, set
\begin{align*}
&\mathcal{C}^1_{\pi,1} = (\Delta x)^{-3},\\
& \mathcal{C}^1_{\pi,2} = (2\pi (\Delta x)^3)^{- \frac{N}{2}}.
\end{align*}
The above, particular choice, will become clear shortly. Let's define a function
\begin{equation}
\delta_{L,N}(\bs{x}) = 
\begin{cases}
