...
\end{equation}
The left side acts on differentiable ``in a good enough sense'' $\phi:\R^4 \to \C^d$, where integer $d$ is not necessarily determined yet and $\gamma^\mu$ is a matrix (i.e. $\gamma^\mu\in M_d(\C))$.
...
\begin{equation}
\label{condition-for-dirac-lambdas}
\boxed{
\{\gamma^\mu, \gamma^\nu\} = 2g^{\mu\nu}
}
\end{equation}

It is sometimes more convenient to see condition (\ref{condition-for-dirac-lambdas}) as split into the conjunction of two
\begin{align}
& \gamma^\mu\gamma^\nu = -\gamma^\nu\gamma^\mu \text{ for } \mu\not=\nu, \\ 
& \gamma^\mu\gamma^\mu = g^{\mu\mu}.
\end{align}

We will attempt now to deduce from (\ref{condition-for-dirac-lambdas}) a few important properties of $\gamma^\mu$. Despite that all of this can be done in more generic way using Clifford algebras, we will currently follow a more elementary approach by Wolfgang Pauli presented in \cite{pauli1936}. 

We will assume a convention where for a given multi-index $\alpha = (\alpha_1, \dots, \alpha_k)$, $\gamma^\alpha\stackrel{def}{=}\gamma^{\alpha_1}\dots\gamma^{\alpha_k}$.
We will call $\alpha$ a multi-index without duplicates iff all indecies in $\alpha$ are pairwise distinct.

\begin{lemma}
\label{dirac-trace-lemma}
Let $\Field$ be a field.
Let $g$ be a diagonal tensor with $g^{\mu\mu}\not=0$. 
Let $d$ be some positive integer and let $\gamma^\mu\in M_d(\Field)$ 
such that $\{\gamma^\mu, \gamma^\nu\} = 2g^{\mu\nu}$ for $\mu, \nu = 0, \dots, n$ where $n \geq 2$ is an even integer, then for any multi-index $\alpha\in(\N\cap[1, n])^n$ such that at least one index appears in $\alpha$ for odd number of times, we have
\begin{equation}
\label{dirac-trace}
tr(\gamma^\alpha) = 0.
\end{equation}
\end{lemma}
\begin{proof}
Let's assume that $\alpha$ is without duplicates.
We will first show that (\ref{dirac-trace}) holds for $|\alpha|$ even. Indeed, assume $\alpha$ is a multi-index without duplicates where $|\alpha|$ is even. Then $\alpha = \beta\mu$ for some $\mu$ and multi-index $\beta$, where $|\beta|$ is odd. Note that $\gamma^\alpha = \gamma^\beta\gamma^\mu = - \gamma^\mu \gamma^\beta$. And since for any two square matrices $A$ and $B$, we have $\tr(AB) = \tr(BA)$ (trace properties are shown in subsection \ref{trace-properties}), it follows that $\tr(\gamma^\alpha)=0$.

Now, let's assume that $\alpha$ is a multi-index without duplicates where $|\alpha|$ is odd. Then since $n$ is even, we always have a multi-index without duplicates $\beta$ such that $\beta = \alpha\mu$ for some index $\mu$. Note that $\gamma^\alpha\gamma^\mu = \gamma^\beta$ and $\gamma^\mu\gamma^\alpha = -\gamma^\beta$. Hence
\begin{equation}
\cfrac{1}{2}\big(\gamma^\beta\gamma^\mu - \gamma^\mu\gamma^\beta\big) = g^{\mu\mu}\gamma^\alpha,
\end{equation}
and consequently $\tr(\gamma^\alpha)=0$.

Now, if $\alpha$ is such that at least one index appers in $\alpha$ for odd number of times, then there exists such an multi-index $\beta$ without duplicates that $\gamma^\alpha = \kappa \gamma^\beta$ such that $\kappa\in \Field\setminus\{0\}$, which completes the proof. 
\end{proof}

For the benefit of the following Lemma we will say that multi-index without duplicates $\alpha$ is significantly different from other multi-index without duplicates $\alpha'$ if there is at least one index $\mu$ which does not belong to both of them.

\begin{lemma}
Let $\Field$ be a field.
Let $g$ be a diagonal tensor with $g^{\mu\mu}\not=0$. 
Let $d$ be some positive integer and let $\gamma^\mu\in M_d(\Field)$ 
such that $\{\gamma^\mu, \gamma^\nu\} = 2g^{\mu\nu}$ for $\mu, \nu = 0, \dots, n$ where $n \geq 2$ is an even integer and let $W\subset (\N\cap[1, n])^n$ be a set of multi-indecies without duplicates pairwaise significantly different, then a set of matrices
\begin{equation}
\{I\}\cup\{\gamma^\alpha:\alpha\in W\}
\end{equation}
is linearly independent and $d \geq 2^{n/2}$.
\end{lemma}
\begin{proof}
We will first show that $\{\gamma^\alpha:\alpha\in W\}$ is linearly independent. Assume that we have coefficients $c_\alpha$ such that
\begin{equation}
0 = \sum_{\alpha\in W} c_\alpha \gamma^\alpha.
\end{equation}
Take any $\beta\in W$, we have
\begin{equation}
0 = \sum_{\alpha\in W\setminus\{\beta\}} c_\alpha \gamma^\alpha \gamma^\beta 
+ \kappa c_\beta I = \sum_{\alpha\in W\setminus\{\beta\}} c_\alpha \gamma^{\alpha\beta} 
+ \kappa c_\beta I,
\end{equation}
where $\kappa\in \Field\setminus\{0\}$. 
Since for all $\alpha\in W\setminus\{\beta\}$, $\alpha$ is significantly different than $\beta$ and both $\alpha$ and $\beta$ are without duplicates, $\alpha\beta$ must be a multi-index with at least one index $\mu$ which appears only once. Hence by Lemma \ref{dirac-trace-lemma}, $\tr(\gamma^{\alpha\beta}) = 0$ for all $\alpha\in W\setminus\{\beta\}$. 
But this implies that $0 = \tr(\kappa c_\beta I) = d\kappa c_\beta$ and hence $c_\beta = 0$. We have shown that $\{\gamma^\alpha:\alpha\in W\}$ is linearly independent. Now, because $\tr(\gamma^\alpha) = 0$ for all $\alpha\in W$, $I$ can't be a linear combination of $\{\gamma^\alpha:\alpha\in W\}$, thus $\{I\}\cup\{\gamma^\alpha:\alpha\in W\}$ is linearly independent. The maximal count of $\{I\}\cup\{\gamma^\alpha:\alpha\in W\}$ must be equal to the number of all subsets of $\N\cap[1, n]$, hence $\dim M_d(\Field) = d^2 \geq 2^n$.
\end{proof}

In case of a Dirac equation $n = 4$, therefore $\gamma^\mu$ must belong to at least $M_4(\C)$.
