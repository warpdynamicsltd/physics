...
For convenience we will define a function $\theta_a$ as follows:
...
\begin{equation}
\int_{-\infty}^\infty f(x) e^{iax}dx = 2\pi i \sum_{k=1}^n \theta_a(\Im z_k) 
\Res(f(z)e^{iaz}, z=z_k).
\end{equation}
\end{lemma}
\begin{proof}
Let's assume that $a > 0$. Take any $R > \max_{k=1}^n |z_k|$. Consider a simple closed path $\Gamma$ from $-R$ to $R$ closed by a half-circle $\Gamma_{R}: \theta \mapsto Re^{i\theta}$ for $\theta\in[0, \pi]$. Since none $z_k$ lies on a line from $-R$ to $R$ and all of them are in $D(0, R)$, by Theorem \ref{cauchy-residue} (Cauchy Residue Theorem) we have:
\begin{equation}
\int_\Gamma f(z) e^{iaz}dz = 2\pi i \sum_{k=1}^n \theta_a(\Im z_k) 
\Res(f(z)e^{iaz}, z=z_k).
\end{equation}
But 
\begin{equation}
...
\end{equation}
and hence thesis.
For $a < 0$, the proof is analogous, with the difference that we would use a half-circle a half-circle $\Gamma_{-R}: \theta \mapsto Re^{i\theta}$ for $\theta\in[\pi, 2\pi]$ and apply Corollary \ref{jordan-lemma-cor}.
\end{proof}
