...
We defined group in Definition \ref{group-definition}. When we will say that $(G, \mathfrak{e})$ is a group (instead of , we will mean that its multiplication operation is denoted as $ab$ for $a, b\in G$.
...
& \sum_{x\in G} \sum_{y\in G} f(x) g(x^{-1}y)h(y^{-1}z) = \sum_{y\in G} (fg)(y) h(y^{-1}z) = \big((fg)h\big)(z).
\end{align*}
Note that
\begin{equation}
1(x)=\begin{cases}
...
Now
\begin{equation}
\mathcal{M}(\mathcal{I}) = \bigcup_{n=0}^\infty \mathcal{M}_n(\mathcal{I}).
\end{equation}

We will now extend multiplication to all $s_1, s_2\in \mathcal{M}(\mathcal{I})$

\begin{equation}
s_1s_2 \stackrel{def}{=} z_1^\frown(ab)^\frown z_2,
\end{equation}
where $s_1 = z_1^\frown a$ and $s_2 = b ^\frown z_2$ for $a,b\in \mathcal{I}$ and $z_1, z_2\in \mathcal{M}(\mathcal{I})$.

We will call $\mathcal{M}(\mathcal{I})$ a set of Clifford multi-indecies iff $\rho(a) = a$ for all $a\in \mathcal{I}$.

%$\mathcal{I} = A \cup \rho(A)$ and $A\cap \rho(A) = \emptyset$.
