...
\begin{theorem}
Let $X$ be a vector space with $\dim X < +\infty$ and let $\Lambda$ be a metric preserivng operator, then $\Lambda^{-1}$ exists and is also metric preserving.
\end{theorem}
...
\begin{theorem}
Let $X$ be a vector space with metric tensor $g$ where $\dim X < +\infty$. $\Lambda$ is metric preserving if and only if
\begin{equation}
...
\end{theorem}
\begin{proof}
Let $e_0, e_1, \dots, e_n$ be orthonormal basis.
Assume $\Lambda$ is metric preserving, then
\begin{align}
g_{\alpha\beta} = g(e_\alpha, e_\beta) = g(\Lambda e_\alpha, \Lambda e_\beta) = 
g_{\mu\nu} (\Lambda e_\alpha)^\mu (\Lambda e_\beta)^\nu = g_{\mu\nu} \Lambda^\mu_\alpha\Lambda^\nu_\beta.
\end{align}
...
Note that property of being orthochronous is base independent. It simply means that $\Lambda$ maps each of the sets from partition into itself.

\begin{lemma}
Let $X$ be a vector space with $\dim X < +\infty$ and $g$ be a metric tensor with signature $(1, n)$ and let $e_0, e_1, \dots, e_n$ be a fixed orthonormal basis. $\Lambda$ is orthohronous iff $\Lambda_0^0 > 0$
\end{lemma}
\begin{proof}
Assume that $\Lambda$ is orthochronous. Note that $e_0 = [1, 0, \dots, 0]$ in the fixed basis. We have $e_0\in \mathcal{T}^{+}$, so $\Lambda e_0 \in \mathcal{T}^{+}$. But $\Lambda^0_0 = (\Lambda  e_0)^0$, hence $\Lambda^0_0 > 0$.\\

Now, assume $\Lambda$ is metric preserving and $\Lambda^0_0 > 0$. Since either $\Lambda \mathcal{T}^{+}\subset \mathcal{T}^{+}$ or $\Lambda \mathcal{T}^{+}\subset \mathcal{T}^{-}$, it is enough that $\Lambda e_0 \in \mathcal{T}^{+}$ for $\Lambda$ to be orthochronous. But $0 < \Lambda^0_0 = (\Lambda  e_0)^0$, hence $\Lambda e_0 \in \mathcal{T}^{+}$.
\end{proof}

Note that for $\Lambda$ metric preserving, by (\ref{metric-preserving-equality}), 
we have $(\Lambda^0_0)^2 - (\bs{\Lambda}_0)^2 = 1$. 
Thus, it is always true that $|\Lambda^0_0| \geq 1$. We can then formulate the following
\begin{corollary}
Let $X$ be a vector space with $\dim X < +\infty$ and $g$ be a metric tensor with signature $(1, n)$ and let $e_0, e_1, \dots, e_n$ be a fixed orthonormal basis. $\Lambda$ is orthohronous iff $\Lambda_0^0 \geq 1$.
\end{corollary}
...
Let $\text{SO}^+(1, n) \subset \text{SO}(1, n)$ be a subgroup of all orthochronous matrices (i.e. with $\Lambda_0^0 \geq 1$). It follows from Thorem \ref{two-sets-of-metric-preserving} that this is indeed subgroup. Note that $\text{SO}^+(1, 3)$ is a group of all restricted Lorentz transformations. This is probably the most important group in modern physics.
