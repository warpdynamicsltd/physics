...
\end{equation}
Moreover $m_0\in E$ and there exists $r\in \R$ such that $\ket{\phi} = r\ket{m}$. 
\end{theorem}
\begin{proof}
...
$e_i = \ket{k/2 + 1 - i}$ for $i=1, \dots, k + 1$. We are now give matrix reprezentation in this basis.
...
Now in an arbitrary space finite dimentional vector space $V$, matrices $S_x, S_y, S_z$ can be described as follows in block notation, using matrices we have worked out above.
