...
To investigate how Weyl transform builds operator $\hat{H}$ investigate $H$ as multilinear polynomial and put in through Corollary \ref{corollary-for-weyl-transform}. 
...
\end{align*}
where $\Delta t = \cfrac{t}{n}$. We will assume (which should be proved more regorously) that in spite of $\Delta t \to 0$ while $n\to 0$, we can replace  $1 - i\Delta tH(p_k, \cfrac{q_{k-1} + q_k}{2})$ with $\exp\big(-i\Delta t H(p_k, \cfrac{q_{k-1} + q_k}{2})\big)$.
