...
I am not sure if I will have time to complete this reasoning, but it will all collapse once Einstein uses synchronisation by slow clock transport. I don't think if anything will violates Lorentz transformation as long as all measurments are done from Einstein point by use of radio waves.\\
...
\section{Tuesday, 14 February 2023}
\subsection{Slow clock transport in Special Relativity Theory}

We would like to analise a proper time of a clock whose equation of motion is given by $t \to (t, t\beta, 0, 0)$ in a chosen frame of reference. We will assume we move the clock from the location $(0, 0, 0, 0)$ to the location $(\Delta t, \Delta x, 0, 0)$. Let's assume without loss of generality that $\Delta x > 0$ and $\beta > 0$. Our main interest will be the time $\Delta\hat{t}$ which the clock shows at the moment of arrival (the proper time of the clock) and how it differs from $\Delta t$ as a function of $\beta$.

From Lorentz transformation, we have
\begin{align*}
\Delta \hat{t} = \gamma \Delta t - \beta\gamma \Delta t\beta =  \gamma (1 - \beta^2) \Delta t = (1 - \beta^2)^{1/2} \Delta t.
\end{align*}
and
\begin{align*}
& \Delta t - \Delta \hat{t} = \Delta t \big( 1 - (1 - \beta^2)^{1/2}\big) = 
\cfrac{\Delta x}{\beta}\cfrac{1 - (1 - \beta^2)}{1 + (1 - \beta^2)^{1/2}} = \\
& \cfrac{\Delta x \beta}{1 + (1 - \beta^2)^{1/2}}.
\end{align*}
Thus, the following (suboptimal by factor of 2 in limit) inequality holds
\begin{equation}
|\Delta t - \Delta \hat{t}| < \Delta x \beta.
\end{equation}

The above inequality is significant for the idea of synchronisation by slow clock transport. Assume that I have two clocks -- A and B at the same point at rest relative to a chosen frame of reference and I want to synchronise clock A with some distant clock C being also at rest relative to the same frame of reference. First I make sure clock $B$ and clock $A$ are synchronised and then move clock $B$ from clock $A$ to clock $C$ and when clock $B$ is at the same place as clock $C$, I can set clock $C$ to the same time as shown by clock $B$. Then clocks $A$ and $C$ will be synchronised with an arbitrary low error, which can be regulated by choosing small enough velocity $\beta$.

\section{Saturday, 25 February 2023}

We defined group in Definition \ref{group-definition}. When we will say that $(G, \mathfrak{e})$ is a group, we will mean that its multiplication operation is denoted as $ab$ for $a, b\in G$ (instead of $a\cdot b$).

\begin{definition}
\label{ring-induced-by-group}
Let $(G, \mathfrak{e})$ be a group and $\Field$ be a field. $\mathcal{F}_0(G, \Field)$ 
is a set of all functions 
$f:G \to \K$ where $f^{-1}(\Field\setminus\{0\})$ is finite.
Morover we define
\begin{equation}
(f + g)(x) \stackrel{def}{=} f(x) + g(x)
\end{equation}
for all $f, g\in \mathcal{F}_0(G, \Field)$, and

\begin{equation}
(fg)(x) = \sum_{u\in G}f(u)g(u^{-1}x)
\end{equation}
for all $f, g\in \mathcal{F}_0(G, \Field)$.
\end{definition}

\begin{theorem}
Let $(G, \mathfrak{e})$ be a group and $\Field$ be a field. Then $\mathcal{F}_0(G, \Field)$ with addition and multiplication defined in Definition \ref{ring-induced-by-group} is a ring.
\end{theorem}

\begin{proof}
It is easy to show from definition that $\mathcal{F}_0(G, \Field)$ forms abelian group with addition.

We will show associavity of multiplication
\begin{align*}
& \big(f(gh)\big)(z) = \sum_{x\in G}  f(x) (gh)(x^{-1}z) = \sum_{x\in G}  f(x) \sum_{u\in G}g(u) h(u^{-1}x^{-1}z) = \\
& \sum_{x\in G} \sum_{u\in G} f(x)  g(u) h(u^{-1}x^{-1}z) \stackrel{y \leftarrow xu}{=}
\sum_{x\in G} \sum_{y\in G}  f(x) g(x^{-1}y)h(y^{-1}x x^{-1} z) = \\
& \sum_{y\in G} \sum_{x\in G} f(x) g(x^{-1}y)h(y^{-1}z) = \sum_{y\in G} (fg)(y) h(y^{-1}z) = \big((fg)h\big)(z).
\end{align*}
Note that
\begin{equation}
1(x)=\begin{cases}
1 &\text{ for } x=\mathfrak{e},\\
0 &\text{ for } x\not=\mathfrak{e}.
\end{cases}
\end{equation}
is a multiplication identity. Also it follows directly from definition that multiplication is left and right distributive with respect to addition.
\end{proof}
\begin{theorem}
If $\mathcal{R}$ is a ring with multiplication and addition and $\Field \subset \mathcal{R}$ is a field with the same multiplication and addition, then $\mathcal{R}$ is also a vector space over $\Field$.
\end{theorem}
Note that all $a1$, where $1$ is a unit multiplication in $\mathcal{F}_0(G, \Field)$, constitute a field isomorphic to $\Field$ and embedded in $\mathcal{F}_0(G, \Field)$ and thus $\mathcal{F}_0(G, \Field)$ is a vector space over this field.\\

\begin{definition}
Let $\mathcal{I}$ will be a set of distinct elements. Let $\mathcal{I}^*$ be a set of all finite strings over $\mathcal{I}$, where $\mathfrak{e}\in \mathcal{I}^*$ is an empty string. Let $\cdot^\frown\cdot : \mathcal{I}^* \times \mathcal{I}^* \to \mathcal{I}^*$ be a concatenation operator. Assume that we have a permutation $\rho: \mathcal{I} \to \mathcal{I}$ such that $\rho^2 = \text{id}$. We will denote $a^{-1} \stackrel{def}{=}\rho(a)$.
We will define a multiplication of elements in $\mathcal{I}$
\begin{equation}
ab \stackrel{def}{=} 
\begin{cases}
\mathfrak{e} &\text{ for } a^{-1} = b,\\
a^\frown b &\text{ for } a^{-1}\not=b.
\end{cases}
\end{equation} 
We will define recursively:
\begin{align*}
& \mathcal{M}_0({\mathcal{I}}) \stackrel{def}{=}  \{\mathfrak{e}\},\\ 
& \mathcal{M}_1({\mathcal{I}}) \stackrel{def}{=}  \mathcal{I},\\
& \mathcal{M}_{n + 1}({\mathcal{I}}) \stackrel{def}{=}
\{ s^\frown(ab) : s^\frown a\in \mathcal{M}_n \text{ and } s\in \mathcal{I}^*
\text{ and } a, b\in \mathcal{I}\} 
\end{align*}
\begin{equation}
\mathcal{M}(\mathcal{I}) \stackrel{def}{=} \bigcup_{n=0}^\infty \mathcal{M}_n(\mathcal{I}).
\end{equation}
We will define multiplication for all $s_1, s_2\in \mathcal{M}(\mathcal{I})$ as
\begin{equation}
s_1s_2 \stackrel{def}{=} z_1^\frown z_2,
\end{equation}
where
\begin{align*}
& s_1 = z_1^\frown a^\frown a_1 ^\frown \dots ^\frown a_n, \\
& s_2 = (a_n^{-1}) ^\frown \dots ^\frown (a_1^{-1})^\frown b^\frown z_2,\\
& \text{and } a \not = b^{-1}
\end{align*}
or
\begin{align*}
& s_1 = z_1^\frown a_1 ^\frown \dots ^\frown a_n, \\
& s_2 = (a_n^{-1}) ^\frown \dots ^\frown (a_1^{-1})^\frown z_2,\\
& \text{and } z_1 = z_2 = \mathfrak{e},
\end{align*}
where $a_1, \dots, a_n\in \mathcal{I}$.
\end{definition}


We will call $\mathcal{M}(\mathcal{I})$ a set of Clifford multi-indecies iff $a^{-1} = a$ for all $a\in \mathcal{I}$.

We will call $\mathcal{M}(\mathcal{I})$ a set of tensor multi-indecies iff $\mathcal{I} = A \cup A^{-1}$ and $A\cap A^{-1} = \emptyset$.

\begin{theorem}
$(\mathcal{M}(\mathcal{I}), \mathfrak{e})$ is a group. 
\end{theorem}

We will call $(\mathcal{M}(\mathcal{I}), \mathfrak{e})$ Clifford multi-index group over indecies $\mathcal{I}$.
